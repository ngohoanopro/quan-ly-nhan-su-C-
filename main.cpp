...
{
private:
  int code, workingTime, position;
  string name, gender, email, numberPone, birthday;

public:
    Employee (int code, string name, string gender, int workingTime, string email, string numberPone, string birthday, int position)
  {
    this->code = code;
    this->name = name;
    this->gender = gender;
 ...
 {
     return employee.totalSalary(employee.workingTime, employee.position);
 }
...
public:
  ...
  Employee createEmployee(int code)
  {
      ...
      Employee employee (code, name, gender, workingTime, email, numberPone, birthday, position);
  ...
  void seederEmployee() {
      for (int i = 0; i <3; i ++) {
          Employee employee (rand(), "user", "boy", 18, "user@gmail.com", "0929045888", "15/06/2000", 1);
          e.push_back (employee);
      }
  }
